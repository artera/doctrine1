...
class JoinCondition extends \Doctrine1\Query\Condition
{
    public function load(string $condition): string
    {
        ...
        if (($l = count($e)) > 2) {
            ...
            if (substr(trim($e[2]), 0, 1) != '(') {
                $expr = new \Doctrine1\Expression($e[2], $this->query->getConnection());
                $e[2] = $expr->getSql();
            }
        ...
        $parser = new \Doctrine1\Query\Where($this->query, $this->tokenizer);
